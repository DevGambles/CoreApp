...
foreach ($a as $b) {
    ...
    for ($i = 24; $i > 0; $i--) {
        ...
        $dresu[$i]['suxx_vs_fail'] = array('successful' => $suxx, 'failed' => $fail, 'date' => date('H:00', $start), 'ts' => new MongoDate($end));
        $dresu[$i]['topups_by_channel'] = array('web' => $ch_web, 'api' => $ch_api, 'pinp' => $ch_pinp, 'ivr' => $ch_ivr, 'date' => date('H:00', $start), 'ts' => new MongoDate($end));
