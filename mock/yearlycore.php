...
$dbw = $connw->selectDB("padb");
                ...
                $current_time = time() ;
                $start = ($current_time - ((86400 * 30) * 12));
                $end = $current_time;
                $obj = array();
...
//                $pipstock = 
                ...
                for ($i = 12; $i > 0; $i--) {
                    if ($i ==1) {
                        $start_mo = $current_time;
                    } else {
                        $start_mo = ($current_time - ((86400 * 30) * $i));
                    }
                    $end_mo = ($start_mo + (86400 * 30));
                    ...
                    $stats = array();
                    $stats['suxx_vs_fail'] = array('ts' => new MongoDate($start), 'successful' => $suxx, 'failed' => $fail, 'date' => date('m.Y', $start_mo));                    
                    $stats['topups_by_channel'] = array('ts' => new MongoDate($start), 'web' => $ch_web, 'api' => $ch_api, 'pinp' => $ch_pinp, 'ivr' => $ch_ivr, 'date' => date('m.Y', $start));
                ...
                $dbw->yearlycorestats->insert($obj);
