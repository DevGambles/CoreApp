...
async function run() {
    ...
    for(let account of accounts) {
        const productCount = await Product.count({account: account._id}).exec();
        let re, wh;
        if(productCount == 0) {
            console.log('NEW ACCOUNT :', account._id);
            ...
            console.log('OLD ACCOUNT : ', account._id, ' SKIPPING! ');
            return;
        }
    }
