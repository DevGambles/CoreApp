...
foreach ($a as $b) {
    ...
    if($w == 0){
        ...
        $end_of_week = strtotime($sundayStr);                    
    } else {
        $end_of_week = strtotime($timestr)+ 86400*(8-$w);                    
    }
    $start_of_week = ($end_of_week - (86400 * 7));
    ...
    $pipe3 = array(
        array('$match' => array(
            ...
            'time' => array('$gte' => new MongoDate($start_of_week), '$lte' => new MongoDate($end_of_week))
        )),
        array('$group' => array(
    ...
    for ($i = 7; $i > 0; $i--) {
        ...
        $dresu[$i]['suxx_vs_fail'] = array('ts' => new MongoDate($start), 'successful' => $suxx, 'failed' => $fail, 'date' => date('d.m.Y', $start));
        $dresu[$i]['topups_by_channel'] = array('ts' => new MongoDate($start), 'web' => $ch_web, 'api' => $ch_api, 'pinp' => $ch_pinp, 'ivr' => $ch_ivr, 'date' => date('d.m.Y', $start));
        
        $dresu[$i]['txvolume'] = array('ts' => new MongoDate($start), 'crd' => $res7c['result'], 'deb' => $res7d['result']);
        $dresu[$i]['bals'] = $bals;
