 ...
 //       if($b['_id']=="5842ec6ab8c649bd5397188f")
        ...
        $res7 = $db->topuplogs->aggregate($pipe7);
			...
			'ts' => new MongoDate($start)
                ...
                $dresu[$i]['suxx_vs_fail'] = array('ts' => new MongoDate($end), 'successful' => $suxx, 'failed' => $fail, 'date' => date('m.Y', $start));
                $dresu[$i]['topups_by_channel'] = array('ts' => new MongoDate($end), 'web' => $ch_web, 'api' => $ch_api, 'pinp' => $ch_pinp, 'ivr' => $ch_ivr, 'date' => date('m.Y', $start));
                
                $dresu[$i]['txvolume'] = array('ts' => new MongoDate($end), 'crd' => $res7c['result'], 'deb' => $res7d['result']);
                $dresu[$i]['bals'] = $bals;
