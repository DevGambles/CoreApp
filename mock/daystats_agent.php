...
foreach ($a as $b) {
    ...
    for($hourDiff = 24; $hourDiff >= 1; $hourDiff --) {
        ...
        $start_of_day = $end_of_day - $hourDiff * 3600;
        $txperday = [];
        $topperday = [];
        $pipe3 = array(
            array('$match' => array(
                'account' => $x['_id'],
                'time' => array('$gte' => new MongoDate($start_of_day), '$lte' => new MongoDate($end_of_day))
            )),
            array('$group' => array(
        ...
        for ($i = $hourDiff; $i > 0; $i--) {
            if(!isset($dcache[$i])) {
                ...
                $dresu['suxx_vs_fail'] = array('successful' => $suxx, 'failed' => $fail, 'date' => date('H:i', $start), 'ts' => new MongoDate($end));
                $dresu['topups_by_channel'] = array('web' => $ch_web, 'api' => $ch_api, 'pinp' => $ch_pinp, 'ivr' => $ch_ivr, 'date' => date('H:i', $start), 'ts' => new MongoDate($end));
        ...
        $ins = array(
            'account' => $x['_id'],
            'time_from' => new MongoDate($start_of_day),
            'time_to' => new MongoDate($end_of_day),
            'time' => new MongoDate(),
