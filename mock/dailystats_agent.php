...
foreach ($a as $b) {
    ...
    $pipe3 = array(
        array('$match' => array(
            'account' => $x['_id'],
            'time' => array('$gte' => new MongoDate($start_of_day), '$lte' => new MongoDate($current_time))
        )),
        array('$group' => array(
    ...
    for ($i = 24; $i > 0; $i--) {
        ...
        $dresu[$i]['suxx_vs_fail'] = array('successful' => $suxx, 'failed' => $fail, 'date' => date('H:i', $start), 'ts' => new MongoDate($end));
        $dresu[$i]['topups_by_channel'] = array('web' => $ch_web, 'api' => $ch_api, 'pinp' => $ch_pinp, 'ivr' => $ch_ivr, 'date' => date('H:i', $start), 'ts' => new MongoDate($end));
    ...
    $ins = array(
        'account' => $x['_id'],
        'time' => new MongoDate($current_time),
