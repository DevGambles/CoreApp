...
$accounts = $db->accounts->find(array('type' => 'agent'));
$accsa = array();
foreach($accounts as $account){
    array_push($accsa,$account['_id']);
}
...
$timestr = "$y-$m-$d $h:00:00";
$end = strtotime($timestr);

echo "Deleting old data".PHP_EOL;

// Delete 3 days old data
$start = $current_time - 259200;
$db->daycorestats->remove(array(
    'time' => array('$lt'=> new MongoDate($start))
));

echo "Deleted old data".PHP_EOL;

echo "Start scanning".PHP_EOL;
// Make statistic until 24 hours ago
$dcache = [];

for($hourDiff = 24; $hourDiff >= 1; $hourDiff --) {
    echo "Scanning hour: " . $hourDiff . PHP_EOL;
    $start = $end - $hourDiff * 3600;
    $obj = array();
    ...
    */
    ...
    $objarray = array();
    for ($i = $hourDiff; $i > 0; $i--) {
        if(!isset($dcache[$i])) {
            $start_mo = ($end - (3600 * $i));
            $end_mo = ($start_mo + 3600);
            $res6Array = array();
            $res7Array = array();
            $res8Array = array();
            $res9Array = array();
            ...
            for ($j=0;$j<sizeof($res2['result']);$j++){
                ...
                for($k = 0;$k<sizeof($res6['result']);$k++){
                    array_push($res6Array,array('_id'=>$res6['result'][$k]['_id']['country'],'count'=>$res6['result'][$k]['count'],
                                    ...
                                    'code' => isset($res6['result'][$k]['_id']['code']) ? $res6['result'][$k]['_id']['code']: null,
                                    'operator_name' => isset($res6['result'][$k]['_id']['operator_name']) ? $res6['result'][$k]['_id']['operator_name']: null,
                ...
                for($k = 0;$k<sizeof($res9['result']);$k++){
                    array_push($res9Array,array('_id'=>$res9['result'][$k]['_id']['country'],'count'=>$res9['result'][$k]['count'],
                            ...
                            'code' => isset($res9['result'][$k]['_id']['code'])? $res9['result'][$k]['_id']['code']: null,
                            'operator_name' => isset($res9['result'][$k]['_id']['operator_name']) ? $res9['result'][$k]['_id']['operator_name']: null,
            ...
            for ($j=0;$j<sizeof($res3['result']);$j++){
                ...
                for($k = 0;$k<sizeof($res7['result']);$k++){
                    array_push($res7Array,array('_id'=>$res7['result'][$k]['_id']['account'],'account_name' => $item['account_name'],'count'=>$res7['result'][$k]['count'],
                                'currency'=>$res7['result'][$k]['_id']['currency'],'amount'=>$res7['result'][$k]['amount'],
                                'code'=> isset($res7['result'][$k]['_id']['code'])? $res7['result'][$k]['_id']['code']: null,
                                'country'=>$res7['result'][$k]['_id']['country'],
                                'operator_name'=>isset($res7['result'][$k]['_id']['operator_name']) ? $res7['result'][$k]['_id']['operator_name']: null,
                ...
                for($k = 0;$k<sizeof($res8['result']);$k++){
                    array_push($res8Array,array('_id'=>$res8['result'][$k]['_id']['account'],'account_name' => $item['account_name'],'count'=>$res8['result'][$k]['count'],
                                'currency'=>$res8['result'][$k]['_id']['currency'],'amount'=>$res8['result'][$k]['amount'],
                                'code'=> isset($res8['result'][$k]['_id']['code'])? $res8['result'][$k]['_id']['code']: null,
                                'country'=>$res8['result'][$k]['_id']['country'],
                                'operator_name'=>isset($res8['result'][$k]['_id']['operator_name']) ? $res8['result'][$k]['_id']['operator_name']: null,
                                'parentid'=>$itemparent['_id'], 'parentname'=>$itemparent['account_name'],
                                'tag' => isset($res8['result'][$k]['_id']['tag'])? $res8['result'][$k]['_id']['tag']: null));
                }
            ...
            $stats = array();
            $stats['suxx_vs_fail'] = array('successful' => $suxx, 'failed' => $fail, 'date' => date('m.Y', $start_mo), 'ts' => new MongoDate($start_mo));                    
            $stats['topups_by_channel'] = array('web' => $ch_web, 'api' => $ch_api, 'pinp' => $ch_pinp, 'ivr' => $ch_ivr, 'date' => date('m.Y', $start), 'ts' => new MongoDate($start_mo));
            ...
            $stats['time'] = date('H:00', $start_mo);
            $dcache[$i] = $stats;
        }
        
        echo "stats['time']" . $stats['time'] . PHP_EOL;
        array_push($objarray, $dcache[$i]);
    }
    $obj['stats'] = $objarray;
    $obj['time_from'] = new MongoDate($start);
    $obj['time_to'] = new MongoDate($end);
    $obj['time'] = new MongoDate($current_time);
    $db->daycorestats->insert($obj);
}
