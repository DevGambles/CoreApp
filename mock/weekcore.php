...
$dbw = $connw->selectDB("padb");
                ...
                if($w == 0){
                    $sundayStr = $y.'-'.$m.'-'.$d.' 24:00:00';
                    $end = strtotime($sundayStr);                    
                } else {
                    $end = strtotime($timestr)+ 86400*(8-$w);                    
                }
                $start = ($end - (86400 * 7));
                $obj = array();
...
//                $pipstock = 
                ...
                for ($i = 7; $i > 0; $i--) {
                    ...
                    $stats = array();
                    $stats['suxx_vs_fail'] = array('ts' => new MongoDate($end_mo), 'successful' => $suxx, 'failed' => $fail, 'date' => date('m.Y', $start_mo));                    
                    $stats['topups_by_channel'] = array('ts' => new MongoDate($end_mo), 'web' => $ch_web, 'api' => $ch_api, 'pinp' => $ch_pinp, 'ivr' => $ch_ivr, 'date' => date('m.Y', $start));
