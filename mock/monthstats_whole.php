...
foreach ($a as $b) {
    ...
    $start_of_month = strtotime($timestr);
    $end_of_month = ($start_of_month + (86400 * $tt) );

    if($d==1){
        $timestr = $y.'-'.($m-1).'-01 00:00:00';
        $start_of_month = strtotime($timestr);
        $timestr = $y.'-'.$m.'-01 00:00:00';
        $end_of_month = strtotime($timestr);
        $tt = date('t',$start_of_month);
    }
    ...
    $pipe3 = array(
        array('$match' => array(
            ...
            'time' => array('$gte' => new MongoDate($start_of_month), '$lte' => new MongoDate($end_of_month))
        )),
        array('$group' => array(
