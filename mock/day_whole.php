...
foreach ($a as $b) {
    ...
    for($hourDiff = 24; $hourDiff >= 1; $hourDiff --) {
        ...
        $start_of_mo = $end_of_mo - $hourDiff * 3600;
        ...
        $pipe3 = array(
            array('$match' => array(
                ...
                'time' => array('$gte' => new MongoDate($start_of_mo), '$lte' => new MongoDate($end_of_mo))
            )),
            array('$group' => array(
        ...
        for ($i = $hourDiff; $i > 0; $i--) {
            if(!isset($dcache[$i])) {
                ...
                $rr = array('sux'=>$ressux['result'],'fail'=>$resfail['result'], 'ts' => new MongoDate($start));
                ...
                $dresu['top5_dest_count'] = $resu;
                $dresu['top5_dest_amount'] = $resu2;
                $dresu['suxx_vs_fail'] = array('successful' => $suxx, 'failed' => $fail, 'date' => date('H:00', $start), 'ts' => new MongoDate($end));
                $dresu['topups_by_channel'] = array('web' => $ch_web, 'api' => $ch_api, 'pinp' => $ch_pinp, 'ivr' => $ch_ivr, 'date' => date('H:00', $start), 'ts' => new MongoDate($end));
                $dresu['txvolume'] = array('crd' => $res7c['result'], 'deb' => $res7d['result'], 'ts' => new MongoDate($end));
                $dresu['bals'] = $bals;
                $dresu['dest_suxx_vs_fail'] = $rr;//dragon added
                $dresu['top_agent'] = $resagentArray;
                $dcache[$i] = $dresu;
            }
            array_push($dstats, $dcache[$i]);
        }
        ...
        $ins = array(
            'account' => $x['_id'],
            'time_from' => new MongoDate($start_of_mo),
            'time_to' => new MongoDate($end_of_mo),
            'time' => new MongoDate(),
